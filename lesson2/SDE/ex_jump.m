...
lam=10; % one year, 10 times, jump times?Cor jump frequency?Galike write 10 reports per year
nt=21*12; % 21 days per month; 12 months
Nt=zeros(nt,1); % jump's times vector
dt=1/nt; % one day 

rng(123456789,'twister');
for t=2:nt % nt - 2 + 1 = nt - 1 times
	if rand(1)<lam*dt % Passion Process?Crandomly generate a double in [0, 1]?C 10*1/(21*12)
		dN=1; % do jump, with a probability of lam*dt
	else
		dN=0; % do not jump, with a probability of (1-lam*dt), specially important
	end
...
for t=2:nt
	...
	Nt(t)=Nt(t-1)+dN; % first update the jumping of Nt(t) based on a jumping probability of lam(t-1)*dt
%	lam(t)=lam(t-1)+...;% <===== 1?s programming
    lam(t) = lam(t-1) + alp * (lambar - lam(t-1))*dt + bt*dN; % then update lam(t) based on updated dN
    % this is a joint jumping process, interesting
end

figure(2);
subplot(2,1,1);plot(lam);grid on;title('Hawkes intensity: $\lambda(t)$','interpreter','latex');
...
figure(3);
subplot(2,1,1);plot(lam);grid on;title('Stochastic Hawkes intensity: $\lambda(t)$','interpreter','latex');
subplot(2,1,2);stairs(Nt);grid on;title('N(t)');
