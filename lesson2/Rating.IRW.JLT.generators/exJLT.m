...
% <<exJLT.m>>
%
% JLT : for Jarrow, Lando and Turnbull 's 1997 paper of 
...
%use Taylor extension for 
% log(1+x) = x - x^2/2 + x^3/3 - x^4/4 + ..., thus we have
% exp(Q) = P, so, 
% Q=log(I+P-I)=(P-I) - (P-I)^2/2 + (P-I)^3/3 - (P-I)^4/4 + ... 
% which is exactly IRW method for computing the "generator" of P. 

np=16; % max degree of polynomial
Q=zeros(size(P));
for i=1:np,
  Q=Q+(-1)^(i-1)*R^i/i; 
  % based on (1.3) page 3 of Theorem 1.1 
  % [Israel-Rosenthal-Wei, IRW method]
end;
disp('Q=');
disp(Q);
...
disp('Mexp(Q)-P=');
Mexp(Q)-P 
% Mexp(Q) should be as close to P as possible, 
% thus Mexp(Q) - P ~= [0]_8*8
disp('Mexp(Q) - P = (should be as close to P as possible)')
disp(Mexp(Q) - P)
disp('Q row-sums')
% ??ˆê—ñ‹?˜a?F
%disp(sum(Q, 1)) 
% sum = [-0.1024,0.0410,0.0645,0.0134,-0.0787,0.0516,-0.3616,0.3737] 
% result is one row vector
% ??ˆê?s‹?˜a?F
disp(sum(Q, 2)) % for each row, sum-up its columns
% sum = [0.0020, -0.0000, -0.0002, -0.0001, -0.0001, -0.0001, 0.0001, 0]'
% before norm(P-exp(Q_{IRW})) =    0.0000000062
fprintf('before norm(P-exp(Q_{IRW}))=%15.10f\n', sum(sum(abs(P-Mexp(Q))))); 
disp('this shows that e^Q is as close to P as possible!');
...
for i=1:nc,
 ...
 k=j(j~=i);
 Q(i,k)=max(QT(i,k),0);
 Q(i,i)=QT(i,i)+sum(min(QT(i,k),0)); 
 % TODO why need to update Q(i,i)? to keep row-sums = 0? by (1.3)
...
% what is IRW? = Israel-Rosenthal-Wei, abbreviation name

figure(1); 
bar3(Mexp(Q)); % 3D bar figure
title('Transition Probability of Ratings in one-year');

figure(2); bar3(Q);      
title('Generator Q (Q is obtained by IRW-method in (1.3)) of Transition Probability');
...
xlabel('rating (7 ranks)');
ylabel('default probability');
legend('1D', '1M', '1Y', '10Y', '35Y','Location','Best'); 
...
title('Transition Probability after 35 years');
xlabel('rating(initial)');
ylabel('rating(final)');
